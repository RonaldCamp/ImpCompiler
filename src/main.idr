...
main = repl "Enter a program: " read_input
  where
    ...
    read_input x = let e = removeJust(ctrlParser (read_token (unpack x) "" [])) in
      let (resp, list) = process ([e], [], empty, empty, []) [] in unwords(map printPi list)
    -- read_input x = show (comands (read_token (unpack x) "" [])) ++ "\n"
    -- read_input x = show (bloc (read_token (unpack x) "" [])) ++ "\n"
